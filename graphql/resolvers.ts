...
export const resolvers = {
    Query: {
        ...
        async getSingleFlashCard(_: any, args: singleFlashCardProp) {
            ...
            if (matches) {
                return { message: `Flashcard related with ${args.flashcard}`, data: { ...matches, createdAt: convertTimestamp(matches?.createdAt) } }
            } else {
        ...
        async getTopics(_: any, args, contextValue: loggedInUser) {
            if (contextValue.user) {
                const userTopic = await prisma.topic.findMany({ where: { userId: contextValue.user.id } })
                return userTopic;
            } else {
    ...
    FlashCard: {
        ...
        Topic: async (_: FlashCardProps) => {
            const related = await prisma.topic.findFirst({ where: { id: _.topicId } });
            return related;
        }
    },
    Topic: {
        User: async (_: TopicProps) => {
            const userTopics = await prisma.topic.findFirst({ where: { id: _.userId } })
            return userTopics;
        },
        FlashCards: async (_: TopicProps) => {
            const related = await prisma.flashCard.findMany({ where: { topicId: _.id } })
            return related;
        },
    },
    Mutation: {
        ...
        async createFlashCard(_: any, args: createFlashCardProps, contextValue: loggedInUser) {
            if (!contextValue.user) return { message: "Please login to create a new topic" }
            const newFC = { reference: args.reference, question: args.question, answer: args.answer, topicId: args.topicId, userId: contextValue.user.id };
            const saveFlashCard = await prisma.flashCard.create({ data: newFC })
            if (saveFlashCard) {
                return { message: "Your flash card has been saved", data: { ...saveFlashCard, createdAt: convertTimestamp(saveFlashCard.createdAt) } }
            } else {
        ...
        async updateFlashCard(_: any, args: editFlashCardProps, contextValue: loggedInUser) {
            ...
            const editedCard = { reference: args.reference, question: args.question, answer: args.answer, topicId: args.topicId }
        ...
        async changeStatus(_: any, args: statusFlashCardProps, contextValue: loggedInUser) {
            ...
            const checkCard = await prisma.flashCard.findFirst({ where: { id: args.fc_id, userId: contextValue.user.id } });
            if (!checkCard) return { message: `We can not find flash card with ${args.fc_id} in your account.` };
            const changeStatus = await prisma.flashCard.update({ where: { id: args.fc_id }, data: { isFlipped: !checkCard.isFlipped } });
            if (changeStatus) return { message: "Status updated successfully.", status: changeStatus.isFlipped }
        },

        // Topic mutatins
        async createNewTopic(_: any, args: CreateTopicProps, contextValue: loggedInUser) {
            if (!contextValue.user) return { message: "Please login to create a new topic" }
            const newTopic: CreateTopicProps = { name: args.name.toLowerCase(), description: args.description, userId: contextValue.user.id }
            const foundMatch = await prisma.topic.findFirst({ where: { name: newTopic.name } }) // check topic existence
            if (foundMatch) return { message: `'${newTopic.name}' already found in your account.` }
            const saveTopic = await prisma.topic.create({ data: newTopic })
            return { message: `'${saveTopic.name}' is saved successfully.`, data: { ...saveTopic, createdAt: convertTimestamp(saveTopic.createdAt) } }
        },
        async editTopic(_: any, args: EditTopicProps, contextValue: loggedInUser) {
            ...
            const checkMatch = await prisma.topic.findFirst({ where: { id: args.tp_id, userId: contextValue.user.id } })
            if (!checkMatch) return { message: "We can not find matching topic with id: " + args.tp_id }
            const updateTopic = await prisma.topic.update({ where: { id: args.tp_id }, data: editedTopic })
        ...
        async deleteTopic(_: any, args: DeleteTopicProps, contextValue: loggedInUser) {
            if (!contextValue.user) return { message: "Please login to create a new topic" }
            const checkMatch = await prisma.topic.findFirst({ where: { id: args.tp_id, userId: contextValue.user.id } })
            if (!checkMatch) return { message: "We can not find matching topic with id: " + args.tp_id + "Or you are not the owner of this topic." }
